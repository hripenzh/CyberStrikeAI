...
<!-- 个人学习用：max_iterations 从 0 改为 20，避免子代理在复杂目标上提前退出循环导致收集不完整。实际使用时可根据目标规模调整。 -->
<!-- 2024-01: 将 max_iterations 从 20 调整为 30，实测大型目标（多子域 + GitHub 泄露扫描）20 轮不够用，30 轮基本够覆盖完整收集流程。小目标可以在调用时手动覆盖为 15。 -->
